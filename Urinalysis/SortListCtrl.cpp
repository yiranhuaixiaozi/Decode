...
void CSortListCtrl::SetHeadItem( int Item,CString Text )
{
	char t_Text[300];
	strcpy(t_Text,Text);
	TCHAR buf[256];
	...
	m_ctlHeader.GetItem( Item, &hditem );   
	hditem.pszText = t_Text;//获取当然列的文字
	m_ctlHeader.SetItem( Item, &hditem );
}

void  CSortListCtrl::SetHeadcolor(COLORREF clrText, COLORREF clrBkgnd)
{
		m_ctlHeader.t_ColorBk = clrBkgnd;
		m_ctlHeader.t_ColorText = clrText;
}
...
COLORREF CSortListCtrl::GetItemColor(int nItem, int nSubItem)
{
	ASSERT(nItem >= 0);
	ASSERT(nItem < GetItemCount());
	if ((nItem < 0) || nItem >= GetItemCount())
	...
	if (pid)return (pid->crText)[nSubItem];
	return -1;
}
